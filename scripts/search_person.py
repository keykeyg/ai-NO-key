...
def main() -> None:
    ...
    parser.add_argument("--start", help="Window start: 21:00 or 2026-08-21 21:00")
    ...
    n = sum(len(t) for t in detection["tracks"].values())
    print(f"Window tracks: {n} on {len(detection['cameras'])} cameras")
    ...
    if detection.get("window", {}).get("start"):
        print(f"Window      : {detection['window']['start']} → {detection['window']['end']}")
