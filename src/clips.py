...
def extract_track_clip(
    ...
    """Cut a short clip of a single track from its source video."""
    if not track.video_path or not track.timestamps or not track.frames:
        return None

    video_path = Path(track.video_path)
    if not video_path.exists():
        logger.warning("Source video missing for track %s: %s", track.track_id, video_path)
        return None
    ...
    # Build a quick lookup of frame → box for drawing
    ...
    writer.release()
    cap.release()
    logger.debug("Wrote clip %s", output_path)
    return output_path
...
def extract_person_clips(
    ...
    """Extract one clip per local track that belongs to a global person."""
    results = []
    person_dir = clips_dir / f"person_{global_id:04d}"
    ...
    for cam, lid in track_refs:
        ...
        if tr is None:
            continue
        out = person_dir / f"{cam}_local{lid}.mp4"
            ...
            results.append(
                {
                    "camera": cam,
                    "local_id": lid,
                    "clip": str(path.relative_to(clips_dir.parent)) if clips_dir.parent in path.parents else str(path),
